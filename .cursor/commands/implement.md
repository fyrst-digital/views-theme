...
The text the user typed after `/implement`.

Resolve the goal in this order:

1. **Non-empty text after `/implement`** — that text is the goal (may refine or override; e.g. “per plan above, also check mobile”).
2. **No extra text** — the goal is the **refined implementation plan already agreed in this session** (conversation Plan-mode output and any plan file path already discussed). Treat that plan as source of truth and implement it fully.
3. **No plan in context and no extra text** (new session, never planned) — ask one clarifying question and stop. Do not invent a large scope.
...
- [docs/conventions/hard-rules.md](../../docs/conventions/hard-rules.md)
- [docs/conventions/agent-workflow.md](../../docs/conventions/agent-workflow.md)
- [AGENTS.md](../../AGENTS.md)
...
| **API / backend / XHR** | Controllers, `/vi/…` routes, PHP services, JSON/HTML endpoints | `curl` against affected URLs (status + body markers). Prefer theme routes from [docs/architecture.md](../../docs/architecture.md). Avoid destructive POSTs unless the goal requires them. |
