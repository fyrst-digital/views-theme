...
| `createAccountChecked` | first load = shop `createCustomerAccountDefault`; posted key = boolean; omitted key on a **filled** bag when `guestSelectable` = guest (`false`) | Switch + toggle open (create-account, not “guest”). Unchecked HTML checkboxes are omitted from POST — do not fall back to shop default on repost. |
...
| `formViolations` | callers | Class component has no `__context` default. `Checkout:Register` reads ambient `__context.formViolations` and passes it; account register bridge passes `:formViolations`. Forwarded to Personal / Form / Credentials / `Privacy:Note`. |
