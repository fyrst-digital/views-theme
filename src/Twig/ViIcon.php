...
class ViIcon extends AbstractExtension
{
    ...
    public function iconMarkup(string $name, ?array $options = []): Markup
    {
        ...
        $ariaHiddenAttr = $ariaHidden ? ' aria-hidden="true"' : '';
        $ariaLabelAttr = $ariaLabel ? ' aria-label="' . \htmlspecialchars($ariaLabel, \ENT_QUOTES, 'UTF-8') . '"' : '';

        $html = '<span class="' . $classString . '"' . $ariaHiddenAttr . $ariaLabelAttr . '>' . $svg . '</span>';
