...
- **New / migrated** components: UX tags `<twig:ViewsTheme:…>`, `{% props %}`, `vi_cva()` / `cva()` + `attributes`, BEM roots with **`vi-`** prefix. See [ux-components.md](docs/conventions/ux-components.md) and [vi-cva.md](docs/twig/vi-cva.md).
